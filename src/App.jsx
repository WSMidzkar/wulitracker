...
function PeriodNav({ mode, onModeChange, period, onPeriodChange }) {
  ...
  const label = mode === "day" ? fmtDate(period) : mode === "month" ? fmtMonth(period) : `Tahun ${period}`;
  const prev = () => onPeriodChange(mode === "day" ? prevDay(period) : mode === "month" ? prevMonth(period) : String(Number(period) - 1));
  const next = () => { if (!atMax) onPeriodChange(mode === "day" ? nextDay(period) : mode === "month" ? nextMonth(period) : String(Number(period) + 1)); };
  ...
  return (
    <div style={{ background: "#fff", borderBottom: `1px solid ${C.border}` }}>
      ...
      <div style={{ display: "flex", alignItems: "center", padding: "6px 8px 10px", gap: 4 }}>
        <button onClick={prev} style={{ width: 34, height: 34, borderRadius: "50%", border: "none", background: C.bg, cursor: "pointer", fontSize: 18, color: C.text, flexShrink: 0 }}>‹</button>
        <div style={{ flex: 1, textAlign: "center", fontSize: 12, fontWeight: 600, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", padding: "0 4px" }}>{label}</div>
        <button onClick={next} disabled={atMax} style={{ width: 34, height: 34, borderRadius: "50%", border: "none", background: atMax ? "transparent" : C.bg, cursor: atMax ? "default" : "pointer", fontSize: 18, color: atMax ? C.border : C.text, flexShrink: 0 }}>›</button>
      </div>
    </div>
  );
}
...
function TodayTab({ habits, save, expenses, journal }) {
  ...
  if (mode === "day") {
    ...
    const todaySpend = expenses.filter(e => e.date === period && e.type === "expense").reduce((s, e) => s + Number(e.amount), 0);
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ marginTop: 8, display: "flex", gap: 6 }}>
                <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 10, padding: "7px 10px", flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700 }}>{fmtRp(todaySpend)}</div>
                </div>
                {rating && (
  ...
  /* ── Month view ── */
  if (mode === "month") {
    const days = Array.from({ length: daysInMonth(period) }, (_, i) => {
      const dk = `${period}-${String(i + 1).padStart(2, "0")}`;
      const h = habits[dk] || {};
      const done = HABITS.filter(hb => h[hb.id]).length;
      return { dk, done, pct: Math.round((done / HABITS.length) * 100), rating: journal[dk]?.rating };
    });
    return (
      <div>
        <div style={{ background: C.primary, paddingTop: safeTop, paddingRight: 20, paddingBottom: 20, paddingLeft: 20, color: "#fff" }}>
            ...
            {days.map(({ dk, done, pct, rating }) => {
              ...
              return (
                <div key={dk} style={{ background: "#fff", borderRadius: 12, padding: "9px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 48, fontSize: 12, color: C.muted, flexShrink: 0 }}>
  ...
  const months = Array.from({ length: 12 }, (_, i) => {
    ...
    for (let d = 1; d <= dim; d++) {
      ...
      totalDone += HABITS.filter(hb => (habits[dk] || {})[hb.id]).length;
    }
    const possible = dim * HABITS.length;
    return { mk, pct: possible ? Math.round((totalDone / possible) * 100) : 0 };
  });
...
function DuitTab({ expenses, save }) {
  ...
  const filterByPeriod = (type) => expenses.filter(e =>
    (mode === "day" ? e.date === period : e.date.startsWith(period)) && e.type === type
  );
  const mExp = filterByPeriod("expense");
  const mInc = filterByPeriod("income");
  ...
  const addEntry = () => {
    ...
    const entry = {
      id: Date.now(), date: tk, type: form.type,
        ...
        <div style={{ padding: "12px 14px 0" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ display: "flex", padding: "8px 14px", borderBottom: `1px solid ${C.border}` }}>
              {["Bulan","Masuk","Keluar","Saldo"].map((h, i) => (
                <span key={h} style={{ flex: i === 0 ? "0 0 36px" : 1, fontSize: 10, fontWeight: 700, color: C.muted, textAlign: i > 0 ? "right" : "left" }}>{h}</span>
              ))}
            </div>
            {yearMonthly.map(({ mk, inc, exp }, i) => {
              ...
              return (
                <div key={mk} style={{ display: "flex", alignItems: "center", padding: "9px 14px", borderTop: i > 0 ? `1px solid ${C.border}` : "none" }}>
                  <span style={{ flex: "0 0 36px", fontSize: 12, fontWeight: 600, color: C.text }}>{MNAMES[i]}</span>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
          ...
          <div style={{ background: "#fff", borderRadius: "22px 22px 0 0", padding: `22px 20px calc(36px + env(safe-area-inset-bottom, 0px))`, width: "100%", maxWidth: 430, position: "relative", zIndex: 1 }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input
                type="number" inputMode="numeric" placeholder="0" value={form.amount}
...
function JurnalTab({ journal, save, habits }) {
  ...
  if (mode === "year") {
    const months = Array.from({ length: 12 }, (_, i) => {
      const mk = `${period}-${String(i + 1).padStart(2, "0")}`;
      const dim = daysInMonth(mk);
      let count = 0, rSum = 0, rCount = 0;
      for (let d = 1; d <= dim; d++) {
        const dk = `${mk}-${String(d).padStart(2, "0")}`;
        if (journal[dk]) {
          count++;
          if (journal[dk].rating) { rSum += journal[dk].rating; rCount++; }
        }
      }
      return { mk, count, avg: rCount ? (rSum / rCount).toFixed(1) : null };
    });
    return (
      <div>
        <div style={{ background: C.positive, paddingTop: safeTop, paddingRight: 20, paddingBottom: 18, paddingLeft: 20, color: "#fff" }}>
...
function WishlistTab({ wishlist, save }) {
  ...
  const add = () => {
    ...
    save({ ...wishlist, [section]: [...items, { id: Date.now(), text: newItem.trim(), done: false }] });
    setNewItem("");
  };
