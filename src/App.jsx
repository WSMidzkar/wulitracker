...
const ECATS = [
  ...
  { id: "tabungan", label: "Tabungan Mau Beli", budget: 0      },
];
...
const fmtDateNum= (d) => new Date(d + "T00:00:00").toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" });
...
function NavBtn({ onClick, disabled, children }) {
  return (
    <button onClick={disabled ? undefined : onClick} style={{
      ...
      background: disabled ? "transparent" : C.bg, cursor: disabled ? "default" : "pointer",
...
function CalendarPopup({ mode, period, onSelect, onClose }) {
  ...
  const jump = (p) => { onSelect(p); onClose(); };
    ...
    return (
      <Overlay onClose={onClose}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 1, marginBottom: 4 }}>
          {DOW.map(d => <div key={d} style={{ textAlign: "center", fontSize: 10, color: C.muted, padding: "3px 0", fontWeight: 600 }}>{d}</div>)}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2 }}>
          {cells.map((d, i) => {
            if (!d) return <div key={i}/>;
            const dk = `${navMonth}-${String(d).padStart(2, "0")}`;
            const isSel = dk === period, isFuture = dk > today, isTd = dk === today;
            return (
              <button key={dk} disabled={isFuture} onClick={() => jump(dk)} style={{
                padding: "7px 2px", borderRadius: 8, border: "none", cursor: isFuture ? "default" : "pointer",
        ...
        <button onClick={() => jump(today)} style={{ marginTop: 14, width: "100%", padding: "10px", borderRadius: 10, border: `1.5px solid ${C.primary}`, background: "transparent", color: C.primary, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Hari Ini</button>
      </Overlay>
    );
  }
      ...
      <Overlay onClose={onClose}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 6 }}>
          {MNAMES.map((mn, i) => {
            ...
            const isSel = mk === period, isFuture = mk > thisMonth;
            return (
              <button key={mk} disabled={isFuture} onClick={() => jump(mk)} style={{
                padding: "11px 4px", borderRadius: 10, border: "none", cursor: isFuture ? "default" : "pointer",
        ...
        <button onClick={() => jump(thisMonth)} style={{ marginTop: 14, width: "100%", padding: "10px", borderRadius: 10, border: `1.5px solid ${C.primary}`, background: "transparent", color: C.primary, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Bulan Ini</button>
      </Overlay>
    );
  }
  ...
  return (
    <Overlay onClose={onClose}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 6 }}>
        {years.map(y => {
          const ys = String(y), isSel = ys === period, isFuture = y > Number(thisYear);
          return (
            <button key={ys} disabled={isFuture} onClick={() => jump(ys)} style={{
              padding: "11px 2px", borderRadius: 10, border: "none", cursor: isFuture ? "default" : "pointer",
              background: isSel ? C.primary : "transparent",
              color: isSel ? "#fff" : isFuture ? C.border : C.text,
              fontSize: 13, fontWeight: isSel ? 700 : 400,
            }}>{y}</button>
      ...
      <button onClick={() => jump(thisYear)} style={{ marginTop: 14, width: "100%", padding: "10px", borderRadius: 10, border: `1.5px solid ${C.primary}`, background: "transparent", color: C.primary, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Tahun Ini</button>
    </Overlay>
  );
}
...
function PeriodNav({ mode, onModeChange, period, onPeriodChange }) {
  ...
  const handleMode = (m) => { onModeChange(m); onPeriodChange(m === "day" ? today : m === "month" ? thisMonth : thisYear); };
    ...
    <>
      <div style={{ background: "#fff", borderBottom: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", padding: "6px 8px 10px", gap: 4 }}>
          ...
          <button onClick={() => setShowCal(true)} style={{ flex: 1, border: "none", background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "4px 0" }}>
      ...
      {showCal && <CalendarPopup mode={mode} period={period} onSelect={onPeriodChange} onClose={() => setShowCal(false)}/>}
    </>
  );
}
...
function usePeriod(defaultMode = "day") {
  ...
  const handleModeChange = (m) => { setMode(m); setPeriod(m === "day" ? todayStr() : m === "month" ? monthStr() : yearStr()); };
...
export default function App() {
  ...
  const persist = (sk, data, setter) => { setter(data); try { localStorage.setItem(sk, JSON.stringify(data)); } catch {} };
  ...
  return (
    <div style={{ maxWidth: 430, margin: "0 auto", minHeight: "100svh", background: C.bg, position: "relative" }}>
      <div style={{ paddingBottom: "calc(64px + env(safe-area-inset-bottom, 0px))" }}>
        ...
        {tab === "wishlist" && <WishlistTab wishlist={wishlist} save={sw} expenses={expenses} saveExpenses={se}/>}
      </div>
      <nav style={{ position: "fixed", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, background: "#fff", borderTop: `1px solid ${C.border}`, display: "flex", zIndex: 100, paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, border: "none", background: "none", padding: "10px 0 8px", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, color: tab === t.id ? C.primary : C.muted, fontSize: 11, fontWeight: tab === t.id ? 700 : 400, cursor: "pointer" }}>
...
function TodayTab({ habits, save, expenses, journal }) {
  ...
  if (mode === "day") {
    const th = habits[period] || {}, done = HABITS.filter(h => th[h.id]).length;
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginTop: 16 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ marginTop: 8, display: "flex", gap: 6 }}>
                ...
                {rating && <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 10, padding: "7px 10px", flexShrink: 0 }}>
        ...
        <div style={{ padding: "12px 14px 0" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            {HABITS.map(h => {
              ...
              return (
                <div key={h.id} onClick={() => toggle(h.id)} style={{ background: isDone ? C.positiveBg : "#fff", border: `1.5px solid ${isDone ? C.positive : C.border}`, borderRadius: 12, padding: "11px 14px", display: "flex", alignItems: "center", gap: 10, cursor: isToday ? "pointer" : "default" }}>
  ...
  if (mode === "month") {
    const days = Array.from({ length: dimOf(period) }, (_, i) => {
      ...
      const h = habits[dk] || {}, dn = HABITS.filter(hb => h[hb.id]).length;
            ...
            {days.map(({ dk, done, pct, rating }) => {
              ...
              return (
                <div key={dk} style={{ background: "#fff", borderRadius: 12, padding: "9px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 50, fontSize: 11, color: C.muted, flexShrink: 0 }}>{d.toLocaleDateString("id-ID",{ weekday:"short", day:"numeric" })}</div>
  ...
  const months = Array.from({ length: 12 }, (_, i) => {
    const mk = `${period}-${String(i+1).padStart(2,"0")}`, dim = dimOf(mk);
    let total = 0;
    for (let d = 1; d <= dim; d++) total += HABITS.filter(hb => (habits[`${mk}-${String(d).padStart(2,"0")}`]||{})[hb.id]).length;
    return { mk, pct: dim ? Math.round((total/(dim*HABITS.length))*100) : 0 };
  });
  return (
    <div>
      <div style={{ background: C.primary, paddingTop: safeTop, paddingRight: 20, paddingBottom: 20, paddingLeft: 20, color: "#fff" }}>
...
function DuitTab({ expenses, save }) {
  ...
  const byPeriod = (type) => expenses.filter(e => (mode === "day" ? e.date === period : e.date.startsWith(period)) && e.type === type);
    ...
    <div>
      <div style={{ background: "#2D2340", paddingTop: safeTop, paddingRight: 20, paddingBottom: 18, paddingLeft: 20, color: "#fff" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          {[{ label:"Pemasukan",val:totalIn,color:C.positive},{label:"Pengeluaran",val:totalOut,color:"#E05C5C"},{label:"Saldo",val:totalIn-totalOut,color:totalIn-totalOut>=0?"#F4C430":"#E05C5C"}].map(s => (
      ...
      {mode !== "year" && (
        <div style={{ padding: "12px 14px 0" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            {ECATS.filter(c => c.budget > 0).map(cat => {
            ...
            {/* Tabungan summary (no budget bar, just total) */}
            {catSpend("tabungan") > 0 && (
              <div style={{ background: "#fff", borderRadius: 12, padding: "11px 14px", display: "flex", justifyContent: "space-between" }}>
                <span style={{ fontSize: 13, fontWeight: 600, color: C.text }}>🐷 Tabungan Mau Beli</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: C.primary }}>{fmtRp(catSpend("tabungan"))}</span>
              </div>
            )}
          </div>
        </div>
          ...
          : <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
            {txList.map(e => (
              <div key={e.id} style={{ background: "#fff", borderRadius: 12, padding: "11px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 20, flexShrink: 0 }}>{e.type==="income"?"📥":e.category==="tabungan"?"🐷":"💸"}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ flexShrink: 0, textAlign: "right" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: e.type==="income"?C.positive:C.danger }}>{e.type==="income"?"+":"-"}{fmtRp(e.amount)}</div>
      ...
      <button onClick={() => setShowAdd(true)} style={{ position: "fixed", bottom: "calc(72px + env(safe-area-inset-bottom, 0px))", right: 16, width: 52, height: 52, borderRadius: "50%", background: C.primary, color: "#fff", border: "none", fontSize: 28, cursor: "pointer", boxShadow: "0 4px 16px rgba(157,92,122,0.35)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>+</button>
        ...
        <BottomSheet onClose={() => setShowAdd(false)}>
          ...
          <div style={{ display: "flex", background: C.bg, borderRadius: 10, padding: 4, marginBottom: 14 }}>
            {["expense","income"].map(t => (
              <button key={t} onClick={() => setForm(f=>({...f,type:t}))} style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "none", cursor: "pointer", background: form.type===t?(t==="income"?C.positive:C.danger):"transparent", color: form.type===t?"#fff":C.muted, fontWeight: 600, fontSize: 13 }}>{t==="expense"?"Pengeluaran":"Pemasukan"}</button>
            ))}
          </div>
          <div style={{ marginBottom: 14 }}>
            ...
            <input type="number" inputMode="numeric" placeholder="0" value={form.amount} onChange={e => setForm(f=>({...f,amount:e.target.value}))}
              style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 22, fontWeight: 700, color: "#1A1A2E", background: "#fff", boxSizing: "border-box", outline: "none" }}/>
          </div>
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 6 }}>{form.type==="expense"?"Kategori":"Sumber"}</label>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {(form.type==="expense" ? ECATS.filter(c=>c.id!=="tabungan").map(c=>({id:c.id,label:c.label})) : ISRC.map(s=>({id:s,label:s}))).map(opt => {
                const isActive = (form.type==="expense"?form.category:form.source)===opt.id;
                return (
                  <button key={opt.id} onClick={() => setForm(f=>({...f,[form.type==="expense"?"category":"source"]:opt.id}))} style={{ padding: "6px 12px", borderRadius: 20, border: `1.5px solid ${isActive?C.primary:C.border}`, background: isActive?C.primaryBg:"#fff", color: isActive?C.primary:C.muted, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>{opt.label}</button>
                );
              })}
            </div>
          </div>
          <div style={{ marginBottom: 18 }}>
            ...
            <input type="text" placeholder="Contoh: makan siang, kopi, dll" value={form.note} onChange={e => setForm(f=>({...f,note:e.target.value}))}
              style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 14, color: "#1A1A2E", background: "#fff", boxSizing: "border-box", outline: "none" }}/>
          </div>
...
function JurnalTab({ journal, save, habits }) {
  ...
  useEffect(() => {
    if (mode === "day") { setDraft(journal[period] || { olahraga:"",makan:"",study:"",catatan:"",rating:7 }); setSaved(false); }
  }, [period, mode]);

  const upd = (f, v) => setDraft(d => ({ ...d, [f]: v }));
  const doSave = () => { save({ ...journal, [period]: draft }); setSaved(true); setTimeout(() => setSaved(false), 2000); };

  if (mode === "month") {
    const dim = dimOf(period), entries = [];
    for (let d = dim; d >= 1; d--) { const dk = `${period}-${String(d).padStart(2,"0")}`; if (journal[dk]) entries.push({ dk, j: journal[dk] }); }
    return (
      <div>
  ...
  if (mode === "year") {
    const months = Array.from({ length: 12 }, (_, i) => {
      ...
      for (let d = 1; d <= dim; d++) { const dk=`${mk}-${String(d).padStart(2,"0")}`; if (journal[dk]) { count++; if(journal[dk].rating){rSum+=journal[dk].rating;rCnt++;} } }
  ...
  return (
    <div>
      <div style={{ background: C.positive, paddingTop: safeTop, paddingRight: 20, paddingBottom: 18, paddingLeft: 20, color: "#fff" }}>
        ...
        {doneBadges.length > 0 && <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 10 }}>{doneBadges.map((icon,i) => <span key={i} style={{ fontSize: 20 }}>{icon}</span>)}</div>}
      </div>
      <PeriodNav mode={mode} onModeChange={handleModeChange} period={period} onPeriodChange={setPeriod}/>
      <div style={{ padding: "12px 14px 0", display: "flex", flexDirection: "column", gap: 10 }}>
        <div style={{ background: "#fff", borderRadius: 14, padding: 14 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.muted, marginTop: 4 }}><span>😔 Jelek</span><span>Keren banget 🤩</span></div>
        </div>
        {[
          ...
          <div key={f} style={{ background: "#fff", borderRadius: 14, padding: "13px 14px" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 8, display: "flex", gap: 7, alignItems: "center" }}><span style={{ fontSize: 17 }}>{icon}</span> {label}</div>
        ...
        <button onClick={doSave} style={{ padding: 15, borderRadius: 14, border: "none", background: saved?C.positive:C.primary, color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>{saved?"✓ Tersimpan!":"Simpan Jurnal"}</button>
        <div style={{ height: 16 }}/>
      </div>
    </div>
  );
...
/* ─── TAB: WISHLIST ─────────────────────────────────────────────────────── */
function WishlistTab({ wishlist, save, expenses, saveExpenses }) {
  const [section,     setSection]     = useState("kebutuhan");
  const [newItem,     setNewItem]     = useState("");
  const [showAddMB,   setShowAddMB]   = useState(false);
  const [mbForm,      setMbForm]      = useState({ text: "", targetPrice: "" });
  const [tabungId,    setTabungId]    = useState(null);
  const [tabungForm,  setTabungForm]  = useState({ amount: "", note: "", date: todayStr() });
  ...
  /* ── helpers for non-mauBeli ── */
  const doneCount = items.filter(i => i.done).length;
  const listPct   = items.length ? Math.round((doneCount/items.length)*100) : 0;
  const addSimple = () => { if (!newItem.trim()) return; save({ ...wishlist, [section]: [...items, { id:Date.now(), text:newItem.trim(), done:false }] }); setNewItem(""); };
  const toggleSimple = (id) => save({ ...wishlist, [section]: items.map(i => i.id===id?{...i,done:!i.done}:i) });
  const delSimple    = (id) => save({ ...wishlist, [section]: items.filter(i => i.id!==id) });

  /* ── helpers for mauBeli ── */
  const mbItems   = wishlist.mauBeli || [];
  const totalSaved = (item) => (item.savings||[]).reduce((s,e)=>s+Number(e.amount),0);
  const savePct    = (item) => item.targetPrice>0 ? Math.min(100,Math.round((totalSaved(item)/item.targetPrice)*100)) : 0;
  const shortfall  = (item) => item.targetPrice>0 ? Math.max(0,item.targetPrice-totalSaved(item)) : 0;

  // Grand totals for summary card
  const grandTarget  = mbItems.filter(i=>!i.done&&i.targetPrice>0).reduce((s,i)=>s+i.targetPrice,0);
  const grandSaved   = mbItems.filter(i=>!i.done).reduce((s,i)=>s+totalSaved(i),0);
  const grandKurang  = Math.max(0,grandTarget-grandSaved);

  const addMauBeli = () => {
    if (!mbForm.text.trim()) return;
    const newMB = { id:Date.now(), text:mbForm.text.trim(), targetPrice:Number(String(mbForm.targetPrice).replace(/[^0-9]/g,""))||0, savings:[], done:false };
    save({ ...wishlist, mauBeli: [...mbItems, newMB] });
    setMbForm({ text:"", targetPrice:"" });
    setShowAddMB(false);
  };

  const addTabungan = () => {
    if (!tabungForm.amount || !tabungId) return;
    const amount    = Number(String(tabungForm.amount).replace(/[^0-9]/g,""));
    const expenseId = Date.now();
    const targetItem = mbItems.find(i => i.id===tabungId);

    const updMB = mbItems.map(item => item.id===tabungId
      ? { ...item, savings:[...(item.savings||[]),{ id:Date.now()+1, amount, date:tabungForm.date, note:tabungForm.note, expenseId }] }
      : item
    );
    save({ ...wishlist, mauBeli: updMB });
    saveExpenses([{ id:expenseId, date:tabungForm.date, type:"expense", amount, category:"tabungan", note:`Nabung: ${targetItem?.text||""}${tabungForm.note?" · "+tabungForm.note:""}` },...expenses]);
    setTabungForm({ amount:"", note:"", date:todayStr() });
    setTabungId(null);
  };

  const delTabungan = (itemId, tabId, expenseId) => {
    const updMB = mbItems.map(item => item.id===itemId ? { ...item, savings:(item.savings||[]).filter(s=>s.id!==tabId) } : item);
    save({ ...wishlist, mauBeli: updMB });
    if (expenseId) saveExpenses(expenses.filter(e => e.id!==expenseId));
  };

  const markBought = (id) => {
    save({ ...wishlist, mauBeli: mbItems.map(i => i.id===id?{...i,done:!i.done}:i) });
  };

  const delMauBeli = (id) => {
    save({ ...wishlist, mauBeli: mbItems.filter(i => i.id!==id) });
  };

  const tabungTarget = mbItems.find(i => i.id===tabungId);
    ...
    <div>
      <div style={{ background: C.accent, paddingTop: safeTop, paddingRight: 20, paddingBottom: 18, paddingLeft: 20 }}>
        ...
        <div style={{ display: "flex", gap: 7, marginTop: 12, flexWrap: "wrap" }}>
          {SECS.map(s => (
            <button key={s.id} onClick={() => setSection(s.id)} style={{ padding: "7px 13px", borderRadius: 20, border: "none", cursor: "pointer", background: section===s.id?"#fff":"rgba(255,255,255,0.3)", color: section===s.id?s.color:"#2D2340", fontWeight: section===s.id?700:500, fontSize: 12 }}>{s.icon} {s.label}</button>
          ))}
        </div>
      </div>

      {/* ── Simple sections: Kebutuhan & Self-Reward ── */}
      {section !== "mauBeli" && (
        <div style={{ padding: "12px 14px 0" }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: "11px 14px", marginBottom: 12, display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              <Ring pct={listPct} size={52} stroke={5} color={cur?.color}/>
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 700, color: cur?.color }}>{listPct}%</div>
            </div>
            <div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            <input type="text" placeholder={`Tambah ${cur?.label.toLowerCase()}...`} value={newItem}
              onChange={e => setNewItem(e.target.value)} onKeyDown={e => e.key==="Enter"&&addSimple()}
            ...
            <button onClick={addSimple} style={{ padding: "11px 16px", borderRadius: 12, border: "none", background: cur?.color, color: "#fff", fontWeight: 700, fontSize: 18, cursor: "pointer" }}>+</button>
          </div>
          {items.length === 0
            ...
            : <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
              {items.map(item => (
                <div key={item.id} style={{ background: "#fff", borderRadius: 12, padding: "11px 14px", display: "flex", alignItems: "center", gap: 10, opacity: item.done?0.6:1 }}>
                  <div onClick={() => toggleSimple(item.id)} style={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0, background: item.done?cur?.color:"transparent", border: `2px solid ${item.done?cur?.color:C.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                  ...
                  <button onClick={() => delSimple(item.id)} style={{ fontSize: 16, color: "#ccc", background: "none", border: "none", cursor: "pointer", padding: "0 2px" }}>✕</button>
                </div>
              ))}
            </div>
          }
      ...
      {/* ── Mau Beli: savings tracker ── */}
      {section === "mauBeli" && (
        <div style={{ padding: "12px 14px 0" }}>

          {/* Grand summary card */}
          {mbItems.filter(i=>!i.done).length > 0 && (
            <div style={{ background: C.primaryBg, borderRadius: 14, padding: "14px 16px", marginBottom: 12, border: `1.5px solid ${C.primaryLight}` }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: C.primary, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 10 }}>Total Nabung</div>
              <div style={{ display: "flex", gap: 8 }}>
                {[
                  { label:"Target",    val:grandTarget, color:C.text    },
                  { label:"Terkumpul", val:grandSaved,  color:C.positive },
                  { label:"Kurang",    val:grandKurang, color:C.danger  },
                ].map(s => (
                  <div key={s.label} style={{ flex: 1, textAlign: "center" }}>
                    <div style={{ fontSize: 10, color: C.muted, marginBottom: 3 }}>{s.label}</div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: s.color }}>{fmtRp(s.val)}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Add button */}
          <button onClick={() => setShowAddMB(true)} style={{ width: "100%", padding: "12px", borderRadius: 12, border: `1.5px dashed ${C.primary}`, background: C.primaryBg, color: C.primary, fontWeight: 700, fontSize: 14, cursor: "pointer", marginBottom: 10 }}>
            + Tambah Barang
          </button>

          {/* Item cards */}
          {mbItems.length === 0
            ? <div style={{ background: "#fff", borderRadius: 12, padding: "20px", textAlign: "center", color: C.muted, fontSize: 14 }}>Belum ada. Tambahin dulu yuk!</div>
            : mbItems.map(item => {
              const saved  = totalSaved(item);
              const pct    = savePct(item);
              const kurang = shortfall(item);
              const isDone = item.done;
              const isExpanded = expandedId === item.id;
              return (
                <div key={item.id} style={{ background: "#fff", borderRadius: 14, padding: "13px 14px", marginBottom: 10, opacity: isDone ? 0.65 : 1, border: isDone?`1.5px solid ${C.positive}`:"none" }}>
                  {/* Item header */}
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: item.targetPrice>0?10:0 }}>
                    <div onClick={() => markBought(item.id)} style={{ width: 22, height: 22, borderRadius: "50%", flexShrink: 0, marginTop: 1, background: isDone?C.positive:"transparent", border: `2px solid ${isDone?C.positive:C.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                      {isDone && <span style={{ color: "#fff", fontSize: 12 }}>✓</span>}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: C.text, textDecoration: isDone?"line-through":"none" }}>{item.text}</div>
                      {item.targetPrice > 0 && <div style={{ fontSize: 12, color: C.muted, marginTop: 1 }}>Target: {fmtRp(item.targetPrice)}</div>}
                    </div>
                    <button onClick={() => delMauBeli(item.id)} style={{ fontSize: 15, color: "#ccc", background: "none", border: "none", cursor: "pointer", padding: "0 2px", flexShrink: 0 }}>✕</button>
                  </div>

                  {/* Progress (if has target) */}
                  {item.targetPrice > 0 && (
                    <>
                      <div style={{ height: 7, background: C.border, borderRadius: 99, marginBottom: 7 }}>
                        <div style={{ height: "100%", width: `${pct}%`, borderRadius: 99, background: pct>=100?C.positive:C.primary, transition:"width 0.4s" }}/>
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                        <span style={{ fontSize: 12, color: C.positive, fontWeight: 600 }}>🐷 {fmtRp(saved)} ({pct}%)</span>
                        {kurang > 0
                          ? <span style={{ fontSize: 12, color: C.danger }}>Kurang {fmtRp(kurang)}</span>
                          : <span style={{ fontSize: 12, color: C.positive, fontWeight: 700 }}>Lunas! 🎉</span>
                        }
                      </div>
                    </>
                  )}

                  {/* No target: just show total saved */}
                  {item.targetPrice === 0 && saved > 0 && (
                    <div style={{ fontSize: 12, color: C.positive, marginBottom: 10 }}>🐷 Terkumpul: {fmtRp(saved)}</div>
                  )}

                  {/* Action buttons */}
                  {!isDone && (
                    <div style={{ display: "flex", gap: 7 }}>
                      <button onClick={() => { setTabungId(item.id); setTabungForm({ amount:"",note:"",date:todayStr() }); }} style={{ flex: 1, padding: "8px 0", borderRadius: 10, border: `1.5px solid ${C.primary}`, background: C.primaryBg, color: C.primary, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>+ Tabung</button>
                      {(item.savings||[]).length > 0 && (
                        <button onClick={() => setExpandedId(isExpanded?null:item.id)} style={{ padding: "8px 12px", borderRadius: 10, border: `1.5px solid ${C.border}`, background: "#fff", color: C.muted, fontSize: 12, cursor: "pointer" }}>
                  ...
                  {/* Savings history */}
                  {isExpanded && (item.savings||[]).length > 0 && (
                    <div style={{ marginTop: 10, borderTop: `1px solid ${C.border}`, paddingTop: 10 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 7 }}>Riwayat Nabung</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        {[...(item.savings||[])].sort((a,b)=>b.id-a.id).map(s => (
                          <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontSize: 13, fontWeight: 600, color: C.positive }}>+{fmtRp(s.amount)}</div>
                              <div style={{ fontSize: 11, color: C.muted }}>{fmtDateNum(s.date)}{s.note?" · "+s.note:""}</div>
                            </div>
                            <button onClick={() => delTabungan(item.id, s.id, s.expenseId)} style={{ fontSize: 13, color: "#ccc", background: "none", border: "none", cursor: "pointer", padding: "0 2px" }}>✕</button>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {/* ── Add Mau Beli modal ── */}
      {showAddMB && (
        <Overlay onClose={() => setShowAddMB(false)}>
          <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 16 }}>Tambah Barang Mau Beli</div>
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Nama Barang</label>
            <input type="text" placeholder="Contoh: Sepatu lari, Skincare set..." value={mbForm.text}
              onChange={e => setMbForm(f=>({...f,text:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&addMauBeli()}
              style={{ width: "100%", padding: "11px 13px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 14, color: "#1A1A2E", background: "#fff", boxSizing: "border-box", outline: "none" }}/>
          </div>
          <div style={{ marginBottom: 20 }}>
            <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Harga Target <span style={{ color: C.border }}>(opsional)</span></label>
            <input type="number" inputMode="numeric" placeholder="0" value={mbForm.targetPrice}
              onChange={e => setMbForm(f=>({...f,targetPrice:e.target.value}))}
              style={{ width: "100%", padding: "11px 13px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 16, fontWeight: 600, color: "#1A1A2E", background: "#fff", boxSizing: "border-box", outline: "none" }}/>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => setShowAddMB(false)} style={{ flex: 1, padding: 13, borderRadius: 12, border: `1.5px solid ${C.border}`, background: "#fff", color: C.muted, fontWeight: 600, cursor: "pointer", fontSize: 14 }}>Batal</button>
            <button onClick={addMauBeli} style={{ flex: 2, padding: 13, borderRadius: 12, border: "none", background: C.primary, color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>Tambah</button>
          </div>
        </Overlay>
      )}

      {/* ── Tabung bottom sheet ── */}
      {tabungId && tabungTarget && (
        <BottomSheet onClose={() => setTabungId(null)}>
          <div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 4 }}>Tambah Tabungan</div>
          <div style={{ fontSize: 13, color: C.muted, marginBottom: 16 }}>untuk: <strong style={{ color: C.primary }}>{tabungTarget.text}</strong></div>

          {tabungTarget.targetPrice > 0 && (
            <div style={{ background: C.primaryBg, borderRadius: 10, padding: "10px 14px", marginBottom: 14, display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontSize: 12, color: C.muted }}>Sudah terkumpul</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: C.primary }}>{fmtRp(totalSaved(tabungTarget))} / {fmtRp(tabungTarget.targetPrice)}</span>
            </div>
          )}

          <div style={{ marginBottom: 13 }}>
            <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Jumlah Tabungan</label>
            <input type="number" inputMode="numeric" placeholder="0" value={tabungForm.amount}
              onChange={e => setTabungForm(f=>({...f,amount:e.target.value}))}
              style={{ width: "100%", padding: "12px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 22, fontWeight: 700, color: "#1A1A2E", background: "#fff", boxSizing: "border-box", outline: "none" }}/>
          </div>
          <div style={{ marginBottom: 13 }}>
            <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Tanggal</label>
            <input type="date" value={tabungForm.date} max={todayStr()}
              onChange={e => setTabungForm(f=>({...f,date:e.target.value}))}
              style={{ width: "100%", padding: "11px 13px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 14, color: "#1A1A2E", background: "#fff", boxSizing: "border-box", outline: "none" }}/>
          </div>
          <div style={{ marginBottom: 18 }}>
            <label style={{ fontSize: 12, color: C.muted, display: "block", marginBottom: 4 }}>Catatan <span style={{ color: C.border }}>(opsional)</span></label>
            <input type="text" placeholder="Dari uang sisa belanja, dll..." value={tabungForm.note}
              onChange={e => setTabungForm(f=>({...f,note:e.target.value}))}
              style={{ width: "100%", padding: "11px 13px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 14, color: "#1A1A2E", background: "#fff", boxSizing: "border-box", outline: "none" }}/>
          </div>
          <div style={{ background: "#FFF8E7", borderRadius: 10, padding: "9px 13px", marginBottom: 16, fontSize: 12, color: "#8B6A00" }}>
            💡 Tabungan ini otomatis tercatat di tab Duit sebagai pengeluaran "Tabungan Mau Beli"
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={() => setTabungId(null)} style={{ flex: 1, padding: 13, borderRadius: 12, border: `1.5px solid ${C.border}`, background: "#fff", color: C.muted, fontWeight: 600, cursor: "pointer", fontSize: 14 }}>Batal</button>
            <button onClick={addTabungan} style={{ flex: 2, padding: 13, borderRadius: 12, border: "none", background: C.primary, color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>Simpan & Catat</button>
          </div>
        </BottomSheet>
      )}
    </div>
